...
function getData(chartIndex, tickers, names) {
	...
	$.each(tickers, function(i, ticker) {
		...
		$.getJSON('/ds/idal?dataonly=true&s1=SEC['+ticker.trim()+']:CLOSE&sdate=09082012&edate=11082013&ctr=yahoo&output=json&debug=true&jsonp1=aa&t='+(new Date().getTime())
...
function addTicker(newTicker){
	$('#chartDiv').append(
				"<div align=\"center\" id=\"container"+(chartN)+"\" style=\"margin:10px; height: <%=heightStr%>px; width: <%=widthStr%>px; float:left;border:2px solid;border-radius:10px;\"></div>"
	);
